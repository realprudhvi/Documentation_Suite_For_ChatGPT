...
The guide below explains how to duplicate the space and connect it to your extension in minutes.

[📄 **Open Private Server Guide (PDF)**](private-server-guide.pdf)

## Limitations

While this extension strives for high fidelity, there are currently a few known limitations:

* **Browser Support:** Currently only supports **Google Chrome** and **Brave**. Firefox and Safari are not yet supported.
* **Flow Charts:** Flow charts and ASCII diagrams inside code blocks may not render correctly in the exported document.
* **Canvas Mode:** Content located within the new "Canvas" interface is not currently detected or supported.
* **Images:** Images generated by DALL-E or uploaded to the chat are **not** currently supported in the export.
